Imports System.Globalization ' Required for CultureInfo
Public Class Form2
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Dim servicePrices As New Dictionary(Of String, Decimal) From {
        {"Catering", 20000D},
        {"Clown", 10000D},
        {"Singer", 7000D},
        {"Dancer", 7000D},
        ...
        Dim pricePerDay As Decimal = 10000D
        Dim totalCost As Decimal = pricePerDay

        ' Use Filipino culture for currency formatting
        Dim phCulture As CultureInfo = New CultureInfo("fil-PH")

        ' Build receipt details
        Dim receipt As New System.Text.StringBuilder()

        receipt.AppendLine("Number Of Guests(50pax): " + tempEventInfo(1) & Environment.NewLine)
        receipt.AppendLine($"Price per day    {pricePerDay.ToString("C", phCulture)}")
        For Each service As String In selectedServices
            If servicePrices.ContainsKey(service) Then
                receipt.AppendLine($"{service.PadRight(18)}{servicePrices(service).ToString("C", phCulture)}")
                totalCost += servicePrices(service)
            End If
        Next

        Dim noOfGuests As Integer
        If Integer.TryParse(tempEventInfo(1), noOfGuests) Then
            Dim multiplier As Integer = Math.Ceiling(noOfGuests / 50)
            totalCost *= multiplier
        End If

        receipt.AppendLine()
        receipt.AppendLine($"Total: {totalCost.ToString("C", phCulture)}")

        Label13.Text = receipt.ToString() & Environment.NewLine & Environment.NewLine & "Please pay before the event date to" & Environment.NewLine & "finalize the booking"
